...
bindgen native/loomc.h \
  ...
  --allowlist-type 'loomc_.*_flag_bits_e' --allowlist-type 'loomc_(context_target_options_t|target_specialization_options_t|target_specialization_t|artifact_manifest_options_t|amdgpu_emit_options_t|amdgpu_profile_execution_options_t)' --allowlist-var 'LOOMC_.*' --no-prepend-enum-name --with-derive-default \
