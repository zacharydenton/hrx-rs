#!/usr/bin/env bash
# Maintainer-only Ubuntu 24.04 build. Source is already pinned and patched.
set -euo pipefail
export DEBIAN_FRONTEND=noninteractive
apt-get update
apt-get install -y --no-install-recommends clang-18 cmake ninja-build git python3 ca-certificates
...
clang-18 --version > /work/hrx-clang/compiler-version.txt
