...
cargo build --manifest-path "$krea/Cargo.toml" --release -p krea2-abi -p krea2-testkit
...
"$scratch/models" "$h3/target/release/libh3.so" "$krea/target/release/libkrea2.so" "$krea/target/release/libnative_ops_test.so"
